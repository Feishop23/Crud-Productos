...
const putMovie  = (req, res) => {
  ...
  if(name && genre && duration && releaseDate){
   moviesControllers.editMovie(id,{name, genre, duration, releaseDate})
     .then((response)=> {
       if(response[0]){
           res.status(200).json({menssage:`Movies with ID: ${id}, editesd succesffully!` })
       }else{
         res.status(404).json({menssage: 'Invalid ID'})
       }
     })
     .catch(err=> {
       res.status(400).json({message: err.message})
  ...
  res.status(400).json({menssage:'Missing data!!!'})
  }
   }
...
const patchMovie = (req, res) => {
  ...
  moviesControllers.editMovie(id, {name, genre, duration, releaseDate})
    ...
    .catch(error => {
      res.status(400).json({message: error.message, text: "cualquiera"})
    })
}
